...
def show_produtos2(request):
    ...
    col2 = bd["carrinho"]
    z = col2.count_documents({})
    return render(request, 'ListCom.html', {"data": x, 'conta': z})
...
def show_destaques(request):
    bd = conexaomongo
    col = bd["produtos"]
    x = col.find({"desconto": {"$gt": 0.00}},{"_id":(0)}).sort("desconto", 1).limit(5)
...
def show_carrinho2(request):
    bd = conexaomongo
    col = bd["carrinho"]
    x = col.find({},{"_id":(0)})
    y = col.aggregate([{"$group": {"_id": 'null', "total": {"$sum": {"$round": [{"$multiply": [{"$toDouble": "$preço"}, "$quantidade", {"$subtract": [1, "$desconto"]}]}, 2]}}}}])
    z = col.count_documents({})
    return render(request, 'Carrinho2.html', {'data': x, 'total': y, 'conta': z})
